...
class GoogleRawFlowService:
    API_URI = "/auth/google-oauth2/callback/"
...
def google_raw_get_credentials() -> GoogleRawFlowService:
    ...
    if not client_id:
        raise ImproperlyConfigured("GOOGLE_OATH_CLIENT_ID is not set")
    if not client_secret:
        raise ImproperlyConfigured("GOOGLE_OATH_CLIENT_SECRET is not set")
    if not project_id:
        raise ImproperlyConfigured("GOOGLE_OATH_PROJECT_ID is not set")
